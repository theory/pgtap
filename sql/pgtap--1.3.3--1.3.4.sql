...
begin
	...
	begin
	    select "returns", lower(langname), returns_set
	...
	if not _returns_set then
		_mock_ddl = format('
	        ...
	        %7$sfunction%7$s;',
			_func_schema/*1*/,  _func_name/*2*/, _func_args/*3*/, _func_result_type/*4*/,
			_return_scalar_value/*5*/, pg_typeof(_return_scalar_value)/*6*/, '$'/*7*/);
	    execute _mock_ddl;
	end if;

    if _returns_set and _func_language not in ('sql', 'plpgsql') then
        raise exception '%', 'It seems that this function type is not supported yet.'
			using HINT = 'Please fill free to open an issue';
    end if;
end $function$;
